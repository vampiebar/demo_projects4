...
$count = 0;
?>
<button data-toggle="modal" href="#newwsr" class="text-left btn btn-info newwsr"><i class="icon-plus"></i> <?php echo $dil["addnewpctimerestriction"]; ?></button>
...
for($i=0;$i<count($xml->pc_run_limited_list);$i++){
	//echo strlen($xml->pc_run_limited_list[11]->pc_run_limit_desc)." ve ".strlen($xml->pc_run_limited_list[12]->pc_run_limit_desc);
?>
<?php if(trim($xml->pc_run_limited_list[$i]->pc_run_limit_desc)==trim($xml->pc_run_limited_list[($i+1)]->pc_run_limit_desc)){ $count++; ?>
<tr class="hiddentr gizli" id="tr<?php echo $xml->pc_run_limited_list[$i]->id;?>">
	<td style="display:none"><?php echo $xml->pc_run_limited_list[$i]->id;?></td>
    <?php if($count>0){ ?>
	<td>
	<?php echo $xml->pc_run_limited_list[$i]->pc_run_limit_desc; ?>
    </td>
    <?php } ?>
    <?php }else{?>
    <tr class="nothiddentr" id="tr<?php echo $xml->pc_run_limited_list[$i]->id;?>">
	<td style="display:none"><?php echo $xml->pc_run_limited_list[$i]->id;?></td>
    <td><?php if($xml->pc_run_limited_list[$i]->limit_weekday!=8){echo "<span>".$xml->pc_run_limited_list[$i]->pc_run_limit_desc."</span><span style=\"font-size:12px; cursor:pointer;\" class=\"label label-info pull-right gizlitr\">".($count+1)." gün</span>"; } else{ echo $xml->pc_run_limited_list[$i]->pc_run_limit_desc; }?></td>
    <?php $count=0;} ?>
    <td><?php echo $xml->pc_run_limited_list[$i]->limit_start_time; ?></td>
    <td><?php echo $xml->pc_run_limited_list[$i]->limit_stop_time; ?></td>
    <td><?php if($xml->pc_run_limited_list[$i]->limit_weekday==1){ echo "Pazartesi"; }if($xml->pc_run_limited_list[$i]->limit_weekday==2){ echo "Salı"; }if($xml->pc_run_limited_list[$i]->limit_weekday==3){ echo "Çarşamba"; }if($xml->pc_run_limited_list[$i]->limit_weekday==4){ echo "Perşembe"; }if($xml->pc_run_limited_list[$i]->limit_weekday==5){ echo "Cuma"; }if($xml->pc_run_limited_list[$i]->limit_weekday==6){ echo "Cumartesi"; }if($xml->pc_run_limited_list[$i]->limit_weekday==0){ echo "Pazar"; } if($xml->pc_run_limited_list[$i]->limit_weekday==8){ echo "Her Zaman"; } ?></td>
    <td style="width:160px;"><?php if($xml->pc_run_limited_list[$i]->pc_run_limit_status=="true"){?><button class="btn btn-danger blockbutton" data-toggle="modal" href="#informallow"><i class="icon-block"></i> <?php echo $dil["blocked"] ?></button><?php }else{?><button class="btn btn-success allowbutton" data-toggle="modal" href="#informblock"><i class="icon-check"></i> <?php echo $dil["allowed"] ?></button><?php } ?></td>
</tr>
<?php
//}
	}
  ...
  <div class="modal-dialog" style="width:30%;">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title text-left"><?php echo $dil["addnewpctimerestriction"] ?></h4>
      </div>
...
$(".addwsr").click(function(a){
		...
		$(".addwsr").addClass("disabled").attr("disabled","disabled");
			$.ajax({
				...
				success:function(){
					$("#newwsr").modal('hide');
					$(".modal-backdrop").hide();
					$(".restrictedpctimes").html('<i style="font-size:50px; color:#0477bd; margin-top:30px; display:inline-block" class="icon-spin2 animate-spin"></i><p style="margin-top:10px;"><?php echo $dil["pls_wait_while_list"] ?></p>');
									...
									if(!proclist){
									$(".restrictedpctimes").html('<?php echo $dil["list_error"] ?>');	
									}else{
									if(proclist=="-2"){
									/* Portu Resetle*/
									...
									*/
									$(".restrictedpctimes").html("Port Canlı Değil");
									}else{
									$(".restrictedpctimes").html(proclist);
									...
									var size = $("restrictedpctimes table tbody tr").size();
									$(".restrictedpctimescount").text(size);
									}
									}
									});
	...
	$(".blockbutton").click(function(){
		var desc = $(this).parent().parent().find("td:eq(1)").find("span:eq(0)").text();
	...
	$(".allowsite").click(function(){
		...
		type:"GET",
		url:"allowpctime.php",
		data:"port="+<?php echo $port; ?>+"&desc="+sitedesc,
		success:function(){
			$("#tr"+siteid).fadeOut();
			$(".allowingurl").hide();
