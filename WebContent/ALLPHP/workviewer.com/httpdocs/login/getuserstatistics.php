...
$(function() {
	$('#reportstartdateusr,#reportenddateusr').datetimepicker({
                   format:"yyyy-MM-dd hh:mm:ss"
    });
	$(".createreportusr").click(function(){
		...
		if(baslangic4>bitis4){
			$(".reportresponseusr").fadeOut();
			$(".tarihuyarireportusr").html('<p>Başlangıç Tarihi Bitiş Tarihinden Küçük Olmalıdır.</p>').fadeIn();
			}else{
				$(".tarihuyarireportusr").fadeOut();
				$(".reportresponseusr").html('<i style="font-size:50px; color:#0477bd; margin-top:30px; display:inline-block" class="icon-spin2 animate-spin"></i> <p style="margin-top:10px;">Rapor Oluşturuluyor. Lütfen bekleyin...</p>').fadeIn();
				$.ajax({
					type:"GET",
					url:"createuserstatisticsreport.php",
					data:"screenport="+port+"&baslangic="+baslangic+"&bitis="+bitis,
					success:function(response){
						$(".reportresponseusr").html(response).fadeIn();
						}
					})
				}
		});
...
<div class='input-group date' id='reportstartdateusr'>
...
<div class='input-group date' id='reportenddateusr'>
...
<div class="form-group col-lg-2">
	<p>&nbsp;</p>
	<button class="btn btn-primary pull-left createreportusr">Raporu Oluştur</button>
</div>
</div>
<div class="row">
<div class="alert alert-danger gizle tarihuyarireportusr"></div>
<div class="reportresponseusr"></div>
</div>