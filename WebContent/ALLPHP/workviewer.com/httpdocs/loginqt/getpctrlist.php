...
$count = 0;
?>
<button data-toggle="modal" href="#newwsr" class="text-left btn btn-info newwsr"><i class="icon-plus"></i> Yeni Bilgisayar Süre Kısıtlaması Ekle</button>
...
<tr class="hiddentr gizli" id="tr<?php echo $xml->pc_run_limited_list[$i]->id;?>">
	...
	<td style="display:none"><?php echo $xml->pc_run_limited_list[$i]->id;?></td>
    <td><?php echo "<span>".$xml->pc_run_limited_list[$i]->pc_run_limit_desc."</span><span style=\"font-size:12px; cursor:pointer;\" class=\"label label-info pull-right gizlitr\">".($count+1)." gün</span>"; ?></td>
    <?php $count=0;} ?>
    <td><?php echo $xml->pc_run_limited_list[$i]->limit_start_time; ?></td>
    <td><?php echo $xml->pc_run_limited_list[$i]->limit_stop_time; ?></td>
    <td><?php if($xml->pc_run_limited_list[$i]->limit_weekday==1){ echo "Pazartesi"; }if($xml->pc_run_limited_list[$i]->limit_weekday==2){ echo "Salı"; }if($xml->pc_run_limited_list[$i]->limit_weekday==3){ echo "Çarşamba"; }if($xml->pc_run_limited_list[$i]->limit_weekday==4){ echo "Perşembe"; }if($xml->pc_run_limited_list[$i]->limit_weekday==5){ echo "Cuma"; }if($xml->pc_run_limited_list[$i]->limit_weekday==6){ echo "Cumartesi"; }if($xml->pc_run_limited_list[$i]->limit_weekday==0){ echo "Pazar"; } ?></td>
  ...
  <div class="modal-dialog" style="width:30%;">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default addwsr">Kuralı Ekle</button>
		...
		$(".addwsr").addClass("disabled").attr("disabled","disabled");
			$.ajax({
				...
				success:function(){
					$("#newwsr").modal('hide');
					$(".modal-backdrop").hide();
					$(".restrictedpctimes").html('<i style="font-size:50px; color:#0477bd; margin-top:30px; display:inline-block" class="icon-spin2 animate-spin"></i><p style="margin-top:10px;">Lütfen Bekleyiniz. Web Sitesi Kısıtlama Listesi Alınıyor...</p>');
									...
									if(!proclist){
									$(".restrictedpctimes").html('Şu anda liste alınamıyor...');	
									}else{
									if(proclist=="-2"){
									/* Portu Resetle*/
