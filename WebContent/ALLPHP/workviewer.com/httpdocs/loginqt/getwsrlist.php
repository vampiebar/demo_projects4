...
$count = 0;
?>
<button data-toggle="modal" href="#newwsr" class="text-left btn btn-info newwsr"><i class="icon-plus"></i> Yeni Site Ekle</button>
...
<tr class="hiddentr gizli" id="tr<?php echo $xml->url_limited_list[$i]->id;?>">
	...
	<td style="display:none"><?php echo $xml->url_limited_list[$i]->id;?></td>
    <td><?php echo $xml->url_limited_list[$i]->url_to_limit."<span style=\"font-size:12px; cursor:pointer;\" class=\"label label-info pull-right gizlitr\">".($count+1)." gün</span>"; ?></td>
    <?php $count=0;} ?>
    <td><?php echo $xml->url_limited_list[$i]->limit_start_time; ?></td>
    <td><?php echo $xml->url_limited_list[$i]->limit_stop_time; ?></td>
    <td><?php if($xml->url_limited_list[$i]->limit_weekday==1){ echo "Pazartesi"; }if($xml->url_limited_list[$i]->limit_weekday==2){ echo "Salı"; }if($xml->url_limited_list[$i]->limit_weekday==3){ echo "Çarşamba"; }if($xml->url_limited_list[$i]->limit_weekday==4){ echo "Perşembe"; }if($xml->url_limited_list[$i]->limit_weekday==5){ echo "Cuma"; } ?></td>
  ...
  <div class="modal-dialog" style="width:30%;">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title text-left">Yeni Site Ekle</h4>
      </div>
      <div class="modal-body">
       <div class="input-group">
          <span class="input-group-addon">http://www.</span>
          <input type="text" class="form-control websiteadress" placeholder="Adresi Formatı:workviewer.com">
        </div>
        ...
        <div class="alert alert-danger gizle addresserror">Lütfen adresi giriniz.</div>
        <div class="alert alert-info gizle addressinfo">Site yasaklanıyor...</div>
        </div>
      </div>
...
<div class="modal fade" id="informallow">
  <div class="modal-dialog" style="width:30%;">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <h4>Bu sitenin yasağını kaldırmak istediğinizden emin misiniz?</h4>
        <input type="hidden" class="siteid" />
        <div class="alert alert-info gizle allowingurl">Site yasağı kaldırılıyor...</div>
      </div>
      <div class="modal-footer">
